...
```
What this code does is handle a new connection event in **line 1**. It grabs this connection, and waits for a new message to be sent by the client in **line 2**.

Once it has a new message, it writes it to the local server console in **line 3**. This is just really for our own debugging and interest. In a production application, perhaps it would save the message logs for later browsing. 

**line 4** is where we send out, or _emit_, the incoming message to all other connected clients, so they can see the message. 
...
Now you can push the the big **Run >** button at the top of the repl to test the server. You should see it installing packages, and output the connection, and finally write _listening on port 3000_
...
Add the [Socket.io-client](https://socket.io/docs/v3/client-installation/index.html)[Socket.io-client](https://socket.io/docs/v3/client-installation/index.html) and [readline module](https://nodejs.org/api/readline.html) to the **index.js** file. 
...
### Create a new connection the chat server

Add a connection to your server repl by adding the line: 
...
1. When we succesfully connect to the chat server, and; 
...
Once we're succesfully connected to the server, we'll want to introduce the user. Add this function to your code: 
...
Now we'll implement the `chat()` function, which waits for a user message and sends it to the server. Add this function to your code: 
...
This code adds a prompt with the user's `chat_handle` to the console, and waits for them to enter a message, through the `chat_interface.question` method. When they enter a message, we prepend it with the user's chat handle and save it to the variable `message_to_send` that we added earlier. Then we send this combined user handle and message to the server using `socket.emit`. Finally we call our chat function again, to setup a prompt and wait for the next message.


### Showing messages from other users

Our last function writes out messages that we receive from the server. We got these messages in code we added earlier that looked like this : `socket.on('broadcast', display_message);`

That code listened for any broadcast messages, and then called the function `display_message`. We'll implement this function now. 

Add the display_message function to your code: 

```javascript
// Handles an incoming message, and checks to see that it is not one we sent. 
...
As you can see in the comments, this function first checks the incoming message to the last message the user sent, stored in the `message_to_send` variable. If they match, the function ignores it and doesn't write it out. It would just look weird if a user send a message, and then had it send right back to themselves!

If it doesn't match (i.e it is a message from another user), then we write it to the console, with a newline `/n` preceding it. Then we go and wait for our user to send a reply by calling our `chat()` function again. 

### Complete client code 

Ok, we are done with the client code. The completed code should look like this: 
...
Now you can push the the big **Run >** button at the top of the client chat repl to test the server. You should see it installing packages, and output the connection, and finally ask _Hello! What's your chat handle?_
...
Now that we've built a server and a client, we can chat with a buddy. Get a friend to copy the client code and run it in their own [repl.it](repl.it) instance so that they can chat with you, or share a link with them to your client chat repl!

Run the server chat app. Then run your client app, and your friend's client app. 

You should see the client apps prompt for your usernames. And after your send them, they will be shown on both clients, and in the server logs. Now you can message each other from the repl console! Once you're comfortable it works with a friend, you can invite others to join you as well. 

## Things to try next

You can try some interesting things to spice up your chat app. 
* Try using differnt colors on the message outputs - one for the user, and another for their chat friends. There are many modules available for this, one of them being [colors](https://www.npmjs.com/package/colors)

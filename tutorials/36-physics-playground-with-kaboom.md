...
Log into your Replit account and create a new repl. Choose Kaboom as your project type. Give this repl a name, like "gravity".

![](/images/tutorials/36-physics-playground/create-repl.png)
...
This code loads a sprite for Kaboom's mascot Bean and places them near the top of the screen. Before we start coding, we'll remove Bean, leaving the following lines:
...
Now we have a blank canvas to work with. We'll start by defining a level containing walls, movable crates and the player object. Add the following code beneath `kaboom();`: 
...
A [game object definition](https://kaboomjs.com/#add) in Kaboom is a list of components, tags, and optionally custom attributes and functions. Components are a core part of Kaboom – they provide different functionality to objects, from an object's appearance to functionality such as collisions.

This wall object has four components:

* [`rect()`](https://kaboomjs.com/#rect), which draws a rectangle to represent the object.
* [`color()`](https://kaboomjs.com/#color), which colors the rectangle.
* [`area()`](https://kaboomjs.com/#area), which provides collision detection for the object.
* [`solid()`](https://kaboomjs.com/#solid), which will prevent other object from moving past it.

We've given the object the tag "wall". Objects can have multiple tags, which can be used to define custom behavior, such as collisions between objects of particular tags.
...
* [`origin()`](https://kaboomjs.com/#origin), which sets the sprite's origin to "center", so that we can move it around from its center, rather than its top-left corner.
...
* As we're using the [`circle`](https://kaboomjs.com/#circle) component to draw the player, we must specify an explicit width and height for the `area()` component. All collision areas in Kaboom.js are rectangular (as of v2000.0.1.6).
* We've added a couple of custom variables to the player object, which we'll use to control its movement speed and jump height.
...
![](/images/tutorials/36-physics-playground/level.png)
...
The `move()` function automatically checks for collisions with solid objects, so the player will only move if there is space to do so.
...
Components are written as functions that return objects of a specific format. Each component should have an `id`, and a list of components it depends on (in `require`). Beyond that, components can have arbitrary attributes, which will be assigned to their parent object, and special functions which will be integrated into the game's [event loop](https://kaboomjs.com/#on).
...
function gravitygun(range, hold, firepower) {
    return {
        ...
        rangeLine: {
            p1: this.pos,
            p2: this.pos.add(pointAt(this.range, this.aimAngle)),
            color: BLACK
        },
        holdLine: {
            p1: this.pos,
            p2: this.pos.add(pointAt(this.hold, this.aimAngle)),
            color: BLUE
        },
    }
}
...
Our gravity gun component is complete. Now we need to add the `physics()` components, for objects affected by the gravity gun. Add the following code just below the last code you added:
...
This component requires both `area` and `body` – to be affected by the gravity gun, objects must both have collision areas and be subject to regular gravity. We also give our physics objects a mass, direction and speed, and several variables to aid the gravity gun. Finally, we define a `draw()` function, which will draw a thick rectangle around the object when it is in gravity gun range.
...
We now have everything in place to write the code that will allow us to pull objects with the gravity gun. We'll do this in an `onUpdate()` event callback for objects with the "movable" tag. Return to the bottom of your file and add the following code:
...
Now let's have the gravity gun pull objects in range when the player holds down the right mouse button. Add the following code below the line where you assigned `movable.inHold`: 
...
![](/images/tutorials/36-physics-playground/gravity-gun-pull.png)
...
![](/images/tutorials/36-physics-playground/wall-clip.png)
...
We defined the `dropping` attribute as part of our `physics()` component, so now all we need to to is make our pull respect the timeout. Find the `if (isMouseDown("right"))` line in your `onUpdate("movable")` event callback and alter it to resemble the following:
